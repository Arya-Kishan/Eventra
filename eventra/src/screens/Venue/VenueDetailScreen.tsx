...
import { CommentType, NavigationProps, RootStackParamList, VenueType } from 'types/AppTypes'
...
const VenueDetailScreen: FC<VenueDetailScreenType> = ({ isCreatingEvent = false }) => {
  ...
  const addComment = async () => {
    ...
    formdata.append("reviews", JSON.stringify({ user: loggedInUser?._id, comment: comment }));
    const { data, success } = await updateVenueApi(formdata, venueDetail!._id);
    if (success) {
      setCommentModal(false)
      setComments("")
    } else {
      showToast({ title: "Not Added !", description: "Some Error Occured", type: "error" })
    }
  }
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>

      {
        ...
        <ScrollView style={{ flex: 1, backgroundColor: AppConstants.screenBgColor }}>
          ...
          <View style={{ padding: AppConstants.screenPadding, gap: vs(6), paddingBottom: vs(60) }}>
            ...
            <CustomImage source={venueDetail.pic.url} width={"100%"} height={vs(250)} />

            <View style={styles.shadowContainer}>

              <View style={{ justifyContent: "space-between", width: "70%", gap: vs(6) }}>
                <CustomText numberOfLines={2} variant='h3' fontWeight='700' >{venueDetail.title}</CustomText>

                <View style={styles.address}>
                  <Icon icon='map-marker' iconType='MaterialCommunityIcons' color={AppConstants.grayColor} size={s(16)} />
                  <CustomText numberOfLines={2} style={{ fontWeight: "500", fontSize: s(16), color: AppConstants.darkGrayColor }} >{`${venueDetail.address.state}, ${venueDetail.address.city}`}</CustomText>
                  <CustomText variant='h4'>|</CustomText>
                  <RoundedBox onPress={() => { openLocationInMaps(Number(venueDetail.location.latitude), Number(venueDetail.location.longitude)); }} size={s(20)}>
            ...
            <View style={[styles.commomShade, { gap: vs(10) }]}>
              ...
              <View>
                ...
                <CustomText variant='body1' numberOfLines={8}>{venueDetail.description}</CustomText>
              </View>
              ...
              <View style={{ flexDirection: "row", gap: s(10), flexWrap: "wrap", justifyContent: "flex-start", marginTop: vs(8) }}>
                {
                  venueDetail.slots.map((item, index) => (
                    <TimeSlot key={index} end={formatISODate(item.time.end).hours.toString()} start={formatISODate(item.time.start).hours.toString()} isBooked={item.isBooked} selected={false} />
                  ))
                }
              </View>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                ...
                <RoundedBox size={s(25)} viewStyle={{ backgroundColor: AppConstants.redColor }} onPress={() => setCommentModal(true)}>
      ...
      >
        <Pressable onPress={() => setCommentModal(false)} style={{ flex: 1, backgroundColor: "#000000D6", justifyContent: "flex-end", alignItems: "flex-end" }}>
          <View style={{ width: "100%", backgroundColor: "#FF0000D6", padding: s(15), borderRadius: s(20), gap: vs(10) }}>

            <CustomText variant='h5' style={{ color: AppConstants.whiteColor }}>Write a comment !</CustomText>

            <TextInput multiline style={{ height: vs(100), textAlignVertical: "top", backgroundColor: AppConstants.whiteColor, borderRadius: s(10), padding: s(10) }} value={comment} onChangeText={setComments} placeholder='assssss' />
