  ...
  return (
    <View
      ...
      <View style={{flexDirection: 'row', gap: s(10)}}>
        ...
        <View style={{justifyContent: 'space-between', gap: s(6)}}>
          <CustomText variant="h5">
            {typeof item.user !== 'string' ? item.user.name : 'User'}
          </CustomText>
          <CustomText style={{fontSize: s(11)}} numberOfLines={2}>
            {item.comment}
          </CustomText>
        </View>
      </View>
      ...
      <View style={{gap: s(6), justifyContent: 'space-between'}}>
        <CustomText
          variant="caption"
          style={{}}>{`${formatDate(item.createdAt!)}`}</CustomText>
        <Pressable
          onPress={deleteComment}
          style={{alignItems: 'flex-end', justifyContent: 'flex-end'}}>
          {loggedInUser?._id ==
          (typeof item.user !== 'string' && item.user._id) ? (
            <Icon
              icon="delete"
