...
import { addUnOpendedMessages, setSelectedOpponentUser, setUnOpendedMessages, setUnSeenMessages } from '@store/reducers/chatSlice'
...
const ChatDashboardScreen = () => {
    ...
    const { unOpenedMessages, unSeenMessages } = useAppSelector(store => store.chat);
    ...
    const handleNavigate = (item: userType) => {
        ...
        dispatch(setUnOpendedMessages(unOpenedMessages.filter((val: MessageType) => (val.sender._id !== item._id))));
        deleteUserUnSeenMessages(item);
    }

    const fetchUnOpnedMessageCount = (user: userType): string => {
        const count = unOpenedMessages.filter((item: MessageType) => (item.sender._id == user._id)).length;
        return count == 0 ? "" : count.toString();
    }

    const fetchAllUnSeenMessages = async () => {
        const { data, success } = await unseenMessageApi({ body: { receiver: loggedInUser?._id }, type: "get" })
    ...
    const deleteUserUnSeenMessages = async (user: userType) => {
        const { data, success } = await unseenMessageApi({ body: { receiver: loggedInUser?._id }, type: "delete" })
    }
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
                ...
                <View style={{ backgroundColor: AppConstants.redColor, padding: AppConstants.screenPadding, flexDirection: "row", gap: s(10), alignItems: "center" }}>
                    ...
                    <CustomText variant='h2' style={{ color: AppConstants.whiteColor }}>Messages</CustomText>
                </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }: { item: userType }) => (
                        <TouchableOpacity
                            ...
                            style={{ flexDirection: "row", gap: s(8), justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <View style={{ flex: 1, flexDirection: "row", gap: s(4), justifyContent: "space-between" }}>
                                ...
                                <View style={{ justifyContent: "space-between", gap: s(10) }}>
                                    ...
                                    <CustomText fontWeight='800' variant='overline'>{isOpponentOnline(item) ? "ONLINE" : "OFFLINE"}</CustomText>
                                </View>

                                <View style={{ justifyContent: "space-between", alignItems: "center", gap: s(10) }}>
                                    {
                                        fetchUnOpnedMessageCount(item) == ""
                                            ?
                                            ""
                                            :
                                            <RoundedBox size={s(20)} onPress={() => { }} viewStyle={{ backgroundColor: AppConstants.redColor }}>
                                                <CustomText variant='overline' fontWeight='700' style={{ color: AppConstants.whiteColor }}>{fetchUnOpnedMessageCount(item)}</CustomText>
                                            </RoundedBox>
                                    }
                                </View>
