...
const getAllSpotLightApi = async (): Promise<ApiReturnType> => {
  try {
    ...
    return {
      data: null,
      message: 'Error in spotLights Fetching',
      success: false,
...
const createspotLightApi = async (
  ...
  try {
    ...
    return {
      data: null,
      message: 'Error in spotLights creating',
      success: false,
      error: JSON.stringify(error),
    }; // Rethrow the error to be handled by the calling function
  }
...
const updatespotLightApi = async (
  spotLight: FormData,
  id: string,
  query?: any,
): Promise<ApiReturnType> => {
...
const getCategoryspotLightApi = async (
  category: string,
): Promise<ApiReturnType> => {
  try {
    const {data} = await axiosInstance.get(
      `/spotLight/category?category=${category}`,
    );
    return {
      data: data,
      message: 'spotLight Fetched',
    ...
    return {
      data: null,
      message: 'Error in spotLight comment fetching',
      success: false,
      error: JSON.stringify(error),
    }; // Rethrow the error to be handled by the calling function
  }
...
export {
  ...
  getAllSpotLightApi,
  updatespotLightApi,
  getSinglespotLightApi,
  getCategoryspotLightApi,
};
