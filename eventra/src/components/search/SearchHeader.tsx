...
const SearchHeader: FC<SearchHeaderProps> = ({
  ...
  return (
    <LinearGradient colors={['#FF0000FF', '#BE0000FF']} style={styles.main}>
      <Animated.View style={[animatedStyle, styles.searchTypeBox]}>
        <View style={[styles.justifyBtn]}>
          <CustomText variant="h2" style={{color: AppConstants.redColor}}>
            Select Search Category
          </CustomText>
          <TouchableOpacity
            activeOpacity={0.5}
            onPress={() => handleAnimate(false)}>
        ...
        <View style={styles.typeContainer}>
          {searchTypes.map(item => (
            <TouchableOpacity
              activeOpacity={0.5}
              onPress={() => handleChangeSearchType(item)}
              key={item}
      ...
      <View style={styles.justifyBtn}>
        <View style={[styles.justifyBtn]}>
          <CustomText
            ...
            }}>{`Search ${searchType}`}</CustomText>
          <Logo width={s(30)} height={s(30)} />
        </View>
        <TouchableOpacity
          activeOpacity={0.5}
          onPress={() => handleAnimate(true)}>
          <Icon
            iconType="Feather"
            icon="menu"
            color={AppConstants.whiteColor}
