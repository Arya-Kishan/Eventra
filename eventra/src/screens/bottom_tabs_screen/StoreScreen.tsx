...
const StoreScreen = () => {
  ...
  const {allEvents, upcomingEvents, eventLoader} = useAppSelector(
    store => store.event,
  );
    ...
    <View style={{flex: 1}}>
      ...
      <Animated.ScrollView style={{flex: 1}}>
        <View style={{gap: vs(10), padding: AppConstants.screenPadding}}>
          <CustomImage
            source="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ6XQdX7t2CB-5y6DAwe95-jTtnWY4BS9iMSOVgDiRsCz31dZFd5uAoXnA9SEAX2asEeXc&usqp=CAU"
            width={'100%'}
            height={vs(100)}
          />
          ...
          <FlatList
            ...
            renderItem={({item}: {item: {pic: string; title: string}}) => (
              <Pressable
                onPress={() => fetchCategoryProducts(item.title)}
                style={[
                  {
                    backgroundColor:
